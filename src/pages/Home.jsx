...
import {
  Zap, ArrowRight, Camera, Map, Bus, Shield, BarChart3, Eye, CheckCircle,
...
export default function Home() {
  const { stats } = useApp();
  const heroVideo = '/videos/hero-mobility.mp4';
      ...
      <section className="relative min-h-[92vh] flex items-center justify-center overflow-hidden pt-16">
        <div className="absolute inset-0 z-0 overflow-hidden">
          <video
            ...
            playsInline
            preload="auto"
            aria-label="Mobility 360 city motion background video"
          />
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[1.15fr_0.85fr]">
            ...
            >
              <motion.span
                ...
                className="badge badge-primary mb-6 inline-flex items-center gap-2"
              >
                <Zap size={11} /> AI-powered smart city mobility platform
              </motion.span>

              <h1 className="hero-title text-white mb-6">
                <span className="hero-title-main">Mangaluru Road &amp; Mobility 360</span>
              </h1>

              <p className="text-lg sm:text-xl text-[#e2e8f0] max-w-xl lg:max-w-2xl mb-8 leading-relaxed">
                Real-time intelligence for safer roads, smarter transit, and a better-connected city — powered by AI and live data.
              </p>

              <div className="flex flex-col sm:flex-row items-center lg:items-start gap-4 mb-8">
                <Link to="/map" className="btn-cta-primary text-base" aria-label="Explore live mobility map">
                  <Map size={18} /> Explore Live Mobility
                </Link>
                <Link to="/report" className="btn-cta-secondary text-base" aria-label="Report a road issue">
                  <Camera size={18} /> Report a Road Issue
                </Link>
              </div>

              <div className="flex flex-wrap items-center justify-center lg:justify-start gap-2.5 text-sm text-[#e2e8f0]">
                {['Live traffic intelligence', 'AI risk detection', 'Bus visibility', 'Citizen-first response'].map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full border border-white/10 bg-white/8 px-3.5 py-1.5 backdrop-blur-sm text-xs tracking-wide"
                  >
                    {tag}
                  </span>
                ))}
            ...
            <motion.div
              ...
              className="relative"
            >
              <div className="hero-visual-shell glass p-3 shadow-[0_30px_80px_rgba(15,23,42,0.7)]">
                <div className="hero-visual-panel">
                  ...
                  <div className="hero-visual-grid">
                    <div className="hero-lane hero-lane-top" />
                    <div className="hero-lane hero-lane-mid" />
                    <div className="hero-lane hero-lane-bottom" />
                    <div className="hero-node hero-node-a" />
                    <div className="hero-node hero-node-b" />
                    <div className="hero-node hero-node-c" />
                    <div className="hero-node hero-node-d" />
                    <div className="hero-node hero-node-e" />
                  </div>

                  <div className="hero-car hero-car-a" />
                  <div className="hero-car hero-car-b" />
                  <div className="hero-bus hero-bus-a" />
                  <div className="hero-alert hero-alert-a" />
                  <div className="hero-alert hero-alert-b" />

                  <div className="hero-floating-card hero-card-primary">
                    <div className="hero-card-label">Live Traffic</div>
                    <div className="hero-card-value">76%</div>
                  </div>
                  <div className="hero-floating-card hero-card-secondary">
                    <div className="hero-card-label">Road Risk</div>
                    <div className="hero-card-value risk">Medium</div>
                  </div>
                  <div className="hero-floating-card hero-card-tertiary">
                    <div className="hero-card-label">Active Buses</div>
                    <div className="hero-card-value">32</div>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-3 mt-4">
                  {[
                    { label: 'Live Traffic', value: '76%', color: '#06B6D4' },
                    { label: 'Active Buses', value: '32', color: '#8B5CF6' },
                    { label: 'Road Risk', value: '12%', color: '#F59E0B' },
                    { label: 'Citizen Reports', value: '48', color: '#10B981' },
                  ].map((metric) => (
                    <div key={metric.label} className="rounded-2xl border border-white/8 bg-slate-950/40 p-3">
                      <div className="text-2xl font-bold" style={{ color: metric.color, fontFamily: 'Space Grotesk, sans-serif' }}>
                        {metric.value}
                      </div>
                      <div className="text-[11px] uppercase tracking-[0.18em] text-[#94A3B8] mt-1">{metric.label}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {heroMetrics.map((metric, index) => (
              ...
              >
                <StatCard icon={<Activity size={16} />} label={metric.label} value={metric.value} color={metric.color} />
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.65 }} className="cta-section text-center">
            <div className="relative z-10">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                <Link to="/report" className="btn-cta-secondary text-base px-9 py-4" aria-label="View live city map">
